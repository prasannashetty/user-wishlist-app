...
def save():
	...
	if red.hgetall(username).keys():
		...
		return render_template('index.html',user_exists=1, 
				msg='(From Redis)',username=username, 
				place=red.hget(username,"place").decode('utf-8'), 
				food=red.hget(username,"food").decode('utf-8'))	
	#if not in redis, check in db
	elif len(red.hgetall(username).keys())==0:
		...
		if record:
			...
			return render_template('index.html',user_exists=1, 
				msg='(From Database)',username=username, 
				place=record.place, food=record.food)

	#if record does not exist in redis and db then insert record
	new_record = userFavs(username=username,place=place,food=food)
	db.session.add(new_record)
	db.commit(new_record)
	...
	return render_template('index.html',saved=1, 
				,username=username, 
				place=red.hget(username,"place").decode('utf-8'), 
				food=red.hget(username,"food").decode('utf-8'))
...
def get():
	...
	if not user_data:
		...
		if not record:
			return render_template('index.html',no_record=1,msg=f"Record not yet
				defined for {{username}}")
		red.hset(username,"place",record.place)
		red.hset(username,"food",record.food)
		return render_template('index.html',get=1,msg="(From Database",
			username=username,place=record.place,food=record.food)
	return render_template('index.html',get=1,msg="(From Redis)",username=username,place=
		user_data[b'place'].decode('utf-8'),food=user_data[b'food'].
		decode('utf-8'))
